...
p_ss_inv <- ggplot(
    ...
    geom_smooth(aes(linetype = group), method = "lm", se = FALSE, alpha = 0.5) +
...
#| fig-width: 4.5
#| fig-height: 3.6
#| fig-cap: Observed scores against true latent factor under invariance (a and b) or partial invariance (c and d).
#| fig-subcap: true
#| layout-ncol: 2
...
pscalar_pars <- lavInspect(pscalar_fit, what = "est")
fscores_reg3 <- list(
    `1` = compute_fscore(
        dat_ex2[dat_ex2$group == 1, 2:4],
        lambda = pscalar_pars[[1]]$lambda,
        theta = pscalar_pars[[1]]$theta,
        nu = pscalar_pars[[1]]$nu,
        psi = 1,
        alpha = 0.5
    ),
    `2` = compute_fscore(
        dat_ex2[dat_ex2$group == 2, 2:4],
        lambda = pscalar_pars[[2]]$lambda,
        theta = pscalar_pars[[2]]$theta,
        nu = pscalar_pars[[2]]$nu,
        psi = 1,
        alpha = 0.5
    )
)
# Add to data
dat_ex2$fsyr <- do.call(rbind, fscores_reg3)
# Plot
yrange <- range(dat_ex2$fsyr)
ycoords <- yrange[1] + diff(yrange) * text_range
lab_df$y <- ycoords
...
## -------------------------------------------------------------------------
#| label: fig-fs-same-prior
#| fig-width: 4.5
#| fig-height: 3.6
#| fig-cap: Regression factor scores against true latent factor under (a) full invariance and (b) partial metric invariance when using the same latent distribution.
#| fig-subcap: true
#| layout-ncol: 2
...
#     c(pars_mimic0$lambda %*% mu_mimic0)
...
## -------------------------------------------------------------------------
#| label: fig-ss-vs-fs-mimic
#| fig-width: 4.5
#| fig-height: 3.6
#| fig-cap: Regression factor scores against true latent factor under MIMIC with (a) invariance and (b) partial scalar invariance.
#| fig-subcap: true
#| layout-ncol: 2
...
#| fig-width: 4.5
#| fig-height: 3.6
#| fig-cap: Expected a posteriori (EAP) scores against true latent factor under full invariance, with (a) group-specific and (b) equal means and variances. The fitted curves are obtained using natural cubic splines.
#| fig-subcap: true
#| layout-ncol: 2
p_eap_inv
p_eap_inv_sp
...
## -------------------------------------------------------------------------
#| label: fig-eap-pinv-sim
#| fig-cap: Expected a posteriori (EAP) scores against true latent factor under partial invariance across 5,000 replications. The vertical bars show the 10th and 90th percentiles, and the lines show the median across replications.
data.frame(
